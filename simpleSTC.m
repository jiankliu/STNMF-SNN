...
%new: input spike train can be both forms

%output 

function [STA,STC,SS,sp_weight,RawMu,RawCov] = simpleSTC(Stim, sp, n, CriticalSize)
...
%   SS  = spike-triggered stimulus matrix
...
if nargout <= 4 
    ...
    if Msz < CriticalSize  % Compute in one chunk if small enough
        ...
        if nargout > 1
            ...
            %STC = SS'*(SS.*repmat(sp(iisp),1,rowlen))/(nsp-1);
        end
        ...
        for j = 2:nchunk
            ...
            SS = makeStimRows(Stim,n,iisp(i0:imx));
            sp_weight = [sp_weight; sp(iisp(i0:imx))];
